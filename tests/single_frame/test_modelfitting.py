...
def test_detection(modelfitting_cross, sim11_r_01_cross):
    """
    Test that the number of results matches the ref, and that they are reasonably close
    """
    assert len(modelfitting_cross.stars_found) >= len(sim11_r_01_cross.stars_found)
    assert len(modelfitting_cross.galaxies_found) >= len(sim11_r_01_cross.galaxies_found)


def test_magnitude(modelfitting_catalog, modelfitting_cross, sim11_r_01_reference, sim11_r_01_cross, tolerances):
    """
    ...
    ref_hits = sim11_r_01_reference[sim11_r_01_cross.all_catalog]
    ...
    ref_mag_diff = ref_mag - sim11_r_01_cross.all_magnitudes
...
def test_generate_report(modelfitting_run, sim11_r_01_reference, sim11_r_simulation, datafiles, module_output_area):
    ...
    with plot.Report(module_output_area / 'report.pdf') as report:
        ...
        dist.add('SExtractor2', sim11_r_01_reference, 'XMODEL_IMAGE', 'YMODEL_IMAGE', marker='o')
        ...
        mag_r.add(
            'SExtractor2',
            sim11_r_01_reference, 'ALPHA_SKY', 'DELTA_SKY', 'MAG_MODEL', 'MAGERR_MODEL',
            marker='o'
        )
        mag_r.add(
            'SourceXtractor++',
        ...
        hist = plot.Histogram(image, sim11_r_simulation)
        hist.add(
            'SExtractor2 MAG_MODEL', sim11_r_01_reference,
            'ALPHA_SKY', 'DELTA_SKY',
            'MAG_MODEL'
        )
        hist.add(
