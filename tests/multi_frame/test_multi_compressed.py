...
def multi_compressed_run(sourcextractor, datafiles, module_output_area, tolerances):
    ...
    run = sourcextractor(
        ...
        python_config_file=datafiles / 'sim12' / 'sim12_multi_compressed.py'
    )
...
def test_iso_flux(multi_compressed_catalog, sim12_r_reference, multi_compressed_cross, sim12_r_cross):
    """
    ...
    """
    catalog_intersect, ref_intersect = intersect(multi_compressed_cross, sim12_r_cross)
    catalog_hits = multi_compressed_catalog[multi_compressed_cross.all_catalog[catalog_intersect]]
...
def test_auto_flux(frame, multi_compressed_catalog, sim12_r_reference, multi_compressed_cross, sim12_r_cross):
    """
...
def test_aper_flux(frame, aper_idx, multi_compressed_catalog, sim12_r_reference, multi_compressed_cross, sim12_r_cross):
    """
...
@pytest.mark.report
def test_generate_report(multi_compressed_run, sim12_r_reference, sim12_r_simulation, datafiles,
                         module_output_area):
    """
    ...
    with plot.Report(module_output_area / 'report.pdf') as report:
        ...
        dist.add('SourceXtractor++', multi_compressed_catalog, 'pixel_centroid_x', 'pixel_centroid_y', marker='.')
        report.add(dist)
        ...
        mag_r.add(
            'SourceXtractor++', multi_compressed_catalog,
        ...
        for i in range(3):
            ...
            flag_r.set_sourcextractor(
                f'SourceXtractor++ auto_flags:{i}', multi_compressed_catalog,
