...
def mef_run(sourcextractor, datafiles, module_output_area, tolerances):
    ...
    run = sourcextractor(
        ...
        python_config_file=datafiles / 'mef' / 'sim12_mef.py'
    )
...
@pytest.fixture(scope='module')
def mef_frame_cross(mef_catalog, sim12_r_simulation, datafiles, tolerances):
    image = Image(
...
def test_iso_flux(mef_catalog, sim12_r_reference, mef_frame_cross, sim12_r_cross):
    """
    ...
    """
    catalog_intersect, ref_intersect = intersect(mef_frame_cross, sim12_r_cross)
...
def test_auto_flux(frame, mef_catalog, sim12_r_reference, mef_frame_cross, sim12_r_cross):
    """
...
def test_aper_flux(frame, aper_idx, mef_catalog, sim12_r_reference, mef_frame_cross, sim12_r_cross):
    """
...
@pytest.mark.report
def test_generate_report(mef_run, sim12_r_reference, sim12_r_simulation, datafiles, module_output_area):
    """
